const infoFromServers = async (client, message, args, name, code) => {

    let r = code.split("$infoFromServers[").length - 1

    let inside = code.split("$infoFromServers[")[r].split("]")[0]

    let fields = inside.split(";")

    let separator = fields[0] || "\n"

    let type = fields[1] || "{name}: {members} members - {memory}KB"
    ...
    code = code.replaceLast(`$infoFromServers[${inside}]`, content.slice(0, 10).join(separator))
...
module.exports = infoFromServers