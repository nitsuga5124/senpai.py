const getAuditLogs = async (client, message, args, name, code) => {
    ...
    if (!message.guild.me.hasPermission("VIEW_AUDIT_LOG") && err === undefined) return message.channel.send(`:x: Failed to fetch logs.`)
    ...
    if (isNaN(entries) || Number(entries) < 1 && err === undefined) return message.channel.send(`:x: Invalid number of entries in \`$getAuditLogs[${inside}]\``)
